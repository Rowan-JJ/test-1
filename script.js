const state = {
  ...
  spus: new Map(), // id -> { id, name, info, skus: [] }
};
...
function createKeyword({ text, heat, rank, color }) {
  const id = `kw_${++keywordCounter}`;
  const keyword = { id, text, heat, rank, color };
...
function createKeywordPill(keyword, { allowRemove, context } = {}) {
  ...
  pill.style.background = keyword.color || '#4c6ef5';
  ...
  if (!allowRemove) {
    ...
    btn.addEventListener('click', () => {
      if (!context) return;
      const { spuId, skuId, keywordId } = context;
...
function handleDragStart(event, { context }) {
  ...
  const payload = context
    ? { type: 'sku', keywordId, skuId: context.skuId, spuId: context.spuId }
...
function handleDrop(event) {
  ...
  if (!payload) return;
  const { type, keywordId, skuId, spuId } = JSON.parse(payload);
  const targetSkuId = dropzone.dataset.skuId;
  const targetSpuId = dropzone.closest('[data-spu-id]').dataset.spuId;
  const spu = state.spus.get(targetSpuId);
  if (!spu) return;
  const targetSku = spu.skus.find((item) => item.id === targetSkuId);
  if (!targetSku) return;

  const dropPositionKeywordId = event.target.closest('.keyword-pill')?.dataset.keywordId;

  if (type === 'library') {
    insertKeywordIntoSku(targetSpuId, targetSkuId, keywordId, dropPositionKeywordId);
  } else if (type === 'sku') {
    if (spuId === targetSpuId && skuId === targetSkuId) {
      reorderKeywordWithinSku(targetSpuId, targetSkuId, keywordId, dropPositionKeywordId);
    } else {
      moveKeywordBetweenSkus(spuId, skuId, targetSpuId, targetSkuId, keywordId, dropPositionKeywordId);
    }
  }
}

function insertKeywordIntoSku(spuId, skuId, keywordId, beforeKeywordId) {
  const spu = state.spus.get(spuId);
  if (!spu) return;
  const sku = spu.skus.find((item) => item.id === skuId);
  if (!sku) return;
  ...
  const insertIndex = beforeKeywordId
    ? sku.titleKeywords.findIndex((id) => id === beforeKeywordId)
    : -1;
  if (insertIndex >= 0) {
    sku.titleKeywords.splice(insertIndex, 0, keywordId);
  } else {
    sku.titleKeywords.push(keywordId);
  }
  renderSpu(spuId);
}

function reorderKeywordWithinSku(spuId, skuId, keywordId, beforeKeywordId) {
  if (keywordId === beforeKeywordId) return;
  const spu = state.spus.get(spuId);
  if (!spu) return;
  const sku = spu.skus.find((item) => item.id === skuId);
  if (!sku) return;
  const currentIndex = sku.titleKeywords.indexOf(keywordId);
  if (currentIndex < 0) return;
  sku.titleKeywords.splice(currentIndex, 1);
  if (beforeKeywordId) {
    const insertIndex = sku.titleKeywords.findIndex((id) => id === beforeKeywordId);
    if (insertIndex >= 0) {
      sku.titleKeywords.splice(insertIndex, 0, keywordId);
    } else {
      sku.titleKeywords.push(keywordId);
...
function moveKeywordBetweenSkus(fromSpuId, fromSkuId, toSpuId, toSkuId, keywordId, beforeKeywordId) {
  const fromSpu = state.spus.get(fromSpuId);
  const toSpu = state.spus.get(toSpuId);
  if (!fromSpu || !toSpu) return;
  const fromSku = fromSpu.skus.find((item) => item.id === fromSkuId);
  const toSku = toSpu.skus.find((item) => item.id === toSkuId);
  if (!fromSku || !toSku) return;
  const index = fromSku.titleKeywords.indexOf(keywordId);
  if (index < 0) return;
  fromSku.titleKeywords.splice(index, 1);
  const insertIndex = beforeKeywordId ? toSku.titleKeywords.indexOf(beforeKeywordId) : -1;
  if (insertIndex >= 0) {
    toSku.titleKeywords.splice(insertIndex, 0, keywordId);
  } else {
    toSku.titleKeywords.push(keywordId);
  }
  renderSpu(fromSpuId);
...
function renderSku(spuId, sku) {
  ...
  if (!sku.titleKeywords.length) {
    dropzone.querySelector('.placeholder').hidden = false;
  } else {
    dropzone.querySelector('.placeholder').hidden = true;
    for (const keywordId of sku.titleKeywords) {
      const keyword = state.keywords.get(keywordId);
      if (!keyword) continue;
      const pill = createKeywordPill(keyword, {
        allowRemove: true,
        context: { spuId, skuId: sku.id, keywordId },
      });
...
function bindSpuEvents(card, spuId) {
  ...
  generateBtn.onclick = () => generateFivePoints(spuId);
}
...
async function generateFivePoints(spuId) {
  ...
  const generateBtn = spuContainer.querySelector(`[data-spu-id="${spuId}"] .generate-five`);
  generateBtn.disabled = true;
  generateBtn.textContent = '生成中...';
  try {
    const prompt = `请根据以下产品信息生成 5 条亚马逊 Listing 五点描述，每条不超过 200 字符：\n${spu.info}`;
    const response = await fetch('https://api.deepseek.com/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'deepseek-chat',
        messages: [
          { role: 'system', content: '你是一名亚马逊运营专家，请用中文输出符合亚马逊要求的五点描述。' },
...
keywordForm.addEventListener('submit', (event) => {
  ...
  createKeyword({
    text,
    heat: formData.get('heat').trim(),
    rank: formData.get('rank').trim(),
    color: formData.get('color'),
  });
  keywordForm.reset();
  keywordForm.querySelector('#keyword-color').value = '#4C6EF5';
});
