...
function sanitizeOptimizedText({
  ...
  return transformTitleText(reordered.join(' '));
}
...
async function generateChildTitles(spuId) {
  ...
  spu.skus.forEach((sku, index) => {
    ...
    updates.push(async () => {
      ...
      words.forEach((word) => {
        const token = nonBrandTokens.find((item) => normalizeWordForMatch(item.text) === normalizeWordForMatch(word));
