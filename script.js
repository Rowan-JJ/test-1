...
async function generateChildTitles(spuId) {
  ...
  spu.skus.forEach((sku, index) => {
    ...
    updates.push(async () => {
      ...
      const gentleShuffle = (list, chance = 0.35) => {
      ...
      const freezeCount = Math.min(3, nonBrandTokens.length);
      const fixedHead = nonBrandTokens.slice(0, freezeCount);
      const adjustable = nonBrandTokens.slice(freezeCount);
      const sceneTail = adjustable.filter((token) => token.type === 'scene');
      const middleTail = adjustable.filter((token) => token.type !== 'scene');

      const sequence = [];
      sequence.push(...brandTokens);
      sequence.push(...fixedHead);
      sequence.push(...gentleShuffle(middleTail));
      sequence.push(...gentleShuffle(sceneTail));
...
async function optimizeSingleTitle(spuId, containerType, containerId, sourceButton) {
  ...
  try {
    ...
    const baseInstruction =
      ...
      '可以适当加入少量连词或介词；除去重复的单词外（单复数算同一个词），不要删去原有的实词，' +
    ...
    if (containerType === 'sku') {
      ...
      const optimizedMiddle = transformTitleText(middleWithRequired);
      ...
      const optimized = transformTitleText(ensureRequiredTokens(originalText, content));
