...
function trimDuplicateTokens(spuId, containerType, containerId) {
  ...
  for (const keyword of tokenKeywords) {
    const key = normalizeWordForMatch(keyword.text || '');
    if (!key) continue;
...
const previewElements = {
  ...
  exportTableBtn: document.getElementById('preview-export-table'),
};
...
function buildExcelTable(headers, rows) {
  ...
  const headerRow = `<tr>${headers.map((cell) => `<th>${escape(cell)}</th>`).join('')}</tr>`;
  const bodyRows = rows
...
function downloadExcelFile(filename, headers, rows) {
  ...
  const html = buildExcelTable(headers, rows);
...
function exportLibraryKeywords() {
  ...
  const rows = ordered.map((keyword) => [
    keyword.text,
    keyword.heat || keyword.searchVolume || '',
    keyword.rank || '',
    getKeywordTypeLabel(keyword.type),
  ]);
  downloadExcelFile('keywords.xls', ['关键词', '周搜索量', '排名', '类型'], rows);
  showToast('关键词库已导出为 Excel 文件');
}
...
function applyFreeTextToContainer(spu, containerType, containerId, text, limit, options = {}) {
  const { forceNeutralColor = false } = options;
  const ownerKey = getContainerKey(spu.id, containerType, containerId);
  cleanupTokensForOwner(ownerKey);
  const words = splitKeywordIntoWords(text);
  const tokenIds = [];
  ...
  for (const word of words) {
    ...
    const color = forceNeutralColor ? baseColor : keyword?.color || baseColor;
    const textColor = forceNeutralColor
      ? defaultTextColor
      : keyword?.textColor || getReadableTextColor(color) || defaultTextColor;
    const tokenId = createTokenFromWord(word, {
      ownerKey,
...
async function generateChildTitles(spuId) {
  ...
  spu.skus.forEach((sku, index) => {
    ...
    updates.push(async () => {
      ...
      const normalizedAllText = normalizeWordForMatch(optimizedText);
      let colorInText = normalizedColor && normalizedAllText.includes(normalizedColor);
      let sizeInText = normalizedSize && normalizedAllText.includes(normalizedSize);
  ...
  try {
    ...
    if (containerType === 'sku') {
      ...
      const combined = [prefixText || 'Popilush', optimizedMiddle, suffixText]
        ...
        .replace(/\s+/g, ' ')
        .trim();
      applyFreeTextToContainer(spu, containerType, containerId, combined, limit, { forceNeutralColor: true });
    } else {
...
function buildPreviewTableRows() {
  ...
  if (!titleBucket?.items?.length && !searchBucket?.items?.length) return [];

  const subtitleMap = new Map();
  const skuTitleMap = new Map();
  const skuSearchMap = new Map();

  for (const item of titleBucket?.items || []) {
    if (!item) continue;
    if (item.type === 'subtitle') {
      subtitleMap.set(item.spuId, item);
    } else if (item.type === 'sku') {
      skuTitleMap.set(`${item.spuId}_${item.skuId}`, item);
    }
  }

  for (const item of searchBucket?.items || []) {
    if (!item) continue;
    skuSearchMap.set(`${item.spuId}_${item.skuId}`, item);
  }

  const rows = [];
  for (const [spuId, spu] of state.spus.entries()) {
    const subtitle = subtitleMap.get(spuId);
    const skuList = Array.isArray(spu?.skus) ? spu.skus : [];
    const hasSkuContent = skuList.some((sku) => {
      const key = `${spuId}_${sku.id}`;
      return skuTitleMap.has(key) || skuSearchMap.has(key);
    });

    if (subtitle || hasSkuContent) {
      rows.push([
        spu.name || spuId,
        subtitle ? transformTitleText(subtitle.text || '') : '',
        '',
      ]);
    }
    ...
    for (const sku of skuList) {
      const key = `${spuId}_${sku.id}`;
      const titleItem = skuTitleMap.get(key);
      const searchItem = skuSearchMap.get(key);
      if (!titleItem && !searchItem) continue;
      rows.push([
        sku.name || sku.id,
        transformTitleText(titleItem?.text || ''),
        transformSearchText(searchItem?.text || ''),
      ]);
    }
  }
  return rows;
...
spuForm.addEventListener('submit', (event) => {
  ...
  if (!name) {
    showToast('请先输入 SPU 名称', true);
    return;
  }
  if (!info) {
    showToast('请先输入商品信息（必填，支持中英文）', true);
    return;
  }
  addSpu({ name, info });
  spuForm.reset();
...
if (previewElements.exportTableBtn) {
  previewElements.exportTableBtn.addEventListener('click', () => {
    if (!state.libraryCollapsed) {
      setLibraryCollapsed(true);
    }
    const rows = buildPreviewTableRows();
    if (!rows.length) {
      showToast('暂无可导出的内容，请先执行文案导出', true);
      return;
    }
    downloadExcelFile('preview-output.xls', ['SKU', '标题', 'Search Term'], rows);
    showToast('已导出 Excel 表格');
  });
}
